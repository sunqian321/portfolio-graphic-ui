...
# ── 参数 ──────────────────────────────────────────────────────────
# 本版（平面 & UI 设计师）的源 PDF。命令行传路径可临时覆盖：
...
SRC_PDF = Path(sys.argv[1]) if len(sys.argv) > 1 else Path(
    "/Users/zaizai/Documents/作品集/投递/作品集pdf/平面+ui.pdf")
...
# ⚠️ 本版的页面顺序和「视觉设计师版」完全不同（章节重排过），这张表是按
# 平面+ui.pdf 重新映射的。判断依据是每页的页高 —— 各页高度互不相同，
# 拿旧版的页高一一对上即可；分隔页是那四张 1080 高的。
PAGES = [
    (1,  "ch0", "cover"),   # 封面
    (2,  "ch0", "about"),   # 关于我（本版这页是 1960×1113，不是 16:9）
    (3,  "ch0", "toc"),     # 目录
    (4,  "ch1", None),      # 01 平面&电商 —— 章节分隔页
    (5,  "ch1", None),      # 包装设计 · 茜越轻运动品牌 VI
    (6,  "ch1", None),      # 电商设计 · 头戴式耳机详情页
    (7,  "ch1", None),      # 图标 & 字体
    (8,  "ch1", None),      # 图标（续）
    (9,  "ch1", None),      # 海报设计
    (10, "ch1", None),      # 海报（续）
    (11, "ch1", None),      # 电商专题 & 活动页
    (12, "ch2", None),      # 02 UI设计 —— 章节分隔页
    (13, "ch2", None),      # 驾享租 · 汽车租赁 APP
    (14, "ch3", None),      # 03 数据可视化 —— 章节分隔页
    (15, "ch3", None),
    (16, "ch4", None),      # 04 APP原型设计 —— 章节分隔页
    (17, "ch4", None),      # 时愈 · 职场健康 APP
    (18, "ch4", None),      # AI 衣橱智搭 APP
